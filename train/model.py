    ...
    def forward(self, img, is_training, keep_prob, name='coarse'):
        with tf.name_scope(name):
            with tf.variable_scope(name):
                # 128(h) 144(w) 3(rgb)
                # (128, 144, 3)
                # conv1, 2 * 2 kernal, 16 kernal (filter) 
                out = self.conv2d('conv1', img, [2, 2, self.input_channle, 16], 1)
                # out is (128, 144, 16)
                ...
                out = tf.reshape(out, [-1, 128 * 16 * 18])
                out = self.make_fc('fc1', out, [128 * 16 * 18, 128], keep_prob)
...
if __name__ == '__main__':
    model = JumpModel()
    out = model.forward(tf.zeros((1, 128, 144, 3)))
